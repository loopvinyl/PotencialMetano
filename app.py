...
if st.session_state.get('run_simulation', False):
    with st.spinner('Executando simulação completa para contexto brasileiro (Tabela 18)...'):
        ...
        # =============================================================================
        # NOVOS GRÁFICOS UTILIZANDO OS DADOS DO SCRIPT
        # =============================================================================
        
        st.markdown("---")
        st.header("📊 Gráficos Baseados nos Dados do Simulador")
        
        # Criar dados para 50 dias (período do artigo)
        dias_artigo = 50
        dias_array = np.arange(1, dias_artigo + 1)
        
        # 1. GRÁFICO DE PERFIS TEMPORAIS DE EMISSÕES DO SCRIPT
        st.subheader("📈 Perfis Temporais de Emissões do Script")
        
        # Calcular emissões usando os dados do script
        # Já temos os perfis PERFIL_CH4_VERMI, PERFIL_N2O_VERMI, etc.
        
        # Calcular emissões totais por lote
        fracao_ms = 1 - umidade
        ch4_vermi_total_por_lote = residuos_kg_dia * (TOC_YANG * CH4_C_FRAC_YANG * (16/12) * fracao_ms)
        n2o_vermi_total_por_lote = residuos_kg_dia * (TN_YANG * N2O_N_FRAC_YANG * (44/28) * fracao_ms)
        
        ch4_thermo_total_por_lote = residuos_kg_dia * (TOC_YANG * CH4_C_FRAC_THERMO * (16/12) * fracao_ms)
        n2o_thermo_total_por_lote = residuos_kg_dia * (TN_YANG * N2O_N_FRAC_THERMO * (44/28) * fracao_ms)
        
        # Distribuir pelos perfis
        ch4_vermi_dia = PERFIL_CH4_VERMI * ch4_vermi_total_por_lote * 1000  # g CH₄/dia
        n2o_vermi_dia = PERFIL_N2O_VERMI * n2o_vermi_total_por_lote * 1000  # g N₂O/dia
        
        ch4_thermo_dia = PERFIL_CH4_THERMO * ch4_thermo_total_por_lote * 1000  # g CH₄/dia
        n2o_thermo_dia = PERFIL_N2O_THERMO * n2o_thermo_total_por_lote * 1000  # g N₂O/dia
        
        # Gráfico 1: Comparação de perfis de CH4
        fig_ch4_comparacao, ax1 = plt.subplots(figsize=(12, 6))
        
        ax1.plot(dias_array, ch4_vermi_dia, 'b-', linewidth=2, label='Vermicompostagem')
        ax1.plot(dias_array, ch4_thermo_dia, 'r-', linewidth=2, label='Compostagem Termofílica')
        ax1.fill_between(dias_array, 0, ch4_vermi_dia, alpha=0.3, color='blue')
        ax1.fill_between(dias_array, 0, ch4_thermo_dia, alpha=0.3, color='red')
        
        ax1.set_xlabel('Dia de Compostagem')
        ax1.set_ylabel('Emissão de CH₄ (g/dia)')
        ax1.set_title('Perfil Temporal de Emissões de CH₄ - Dados do Script')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        # Adicionar valores totais
        total_ch4_vermi = ch4_vermi_dia.sum()
        total_ch4_thermo = ch4_thermo_dia.sum()
        reducao_ch4 = ((total_ch4_thermo - total_ch4_vermi) / total_ch4_thermo) * 100
        
        ax1.text(0.02, 0.98, f'Total Vermicompostagem: {total_ch4_vermi:.1f} g CH₄/lote\n'
                              f'Total Termofílica: {total_ch4_thermo:.1f} g CH₄/lote\n'
                              f'Redução: {reducao_ch4:.1f}%',
                transform=ax1.transAxes, verticalalignment='top',
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))
        
        st.pyplot(fig_ch4_comparacao)
        
        # Gráfico 2: Comparação de perfis de N2O
        fig_n2o_comparacao, ax2 = plt.subplots(figsize=(12, 6))
        
        ax2.plot(dias_array, n2o_vermi_dia, 'b-', linewidth=2, label='Vermicompostagem')
        ax2.plot(dias_array, n2o_thermo_dia, 'r-', linewidth=2, label='Compostagem Termofílica')
        ax2.fill_between(dias_array, 0, n2o_vermi_dia, alpha=0.3, color='blue')
        ax2.fill_between(dias_array, 0, n2o_thermo_dia, alpha=0.3, color='red')
        
        ax2.set_xlabel('Dia de Compostagem')
        ax2.set_ylabel('Emissão de N₂O (g/dia)')
        ax2.set_title('Perfil Temporal de Emissões de N₂O - Dados do Script')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        # Adicionar valores totais
        total_n2o_vermi = n2o_vermi_dia.sum()
        total_n2o_thermo = n2o_thermo_dia.sum()
        reducao_n2o = ((total_n2o_thermo - total_n2o_vermi) / total_n2o_thermo) * 100
        
        ax2.text(0.02, 0.98, f'Total Vermicompostagem: {total_n2o_vermi:.1f} g N₂O/lote\n'
                              f'Total Termofílica: {total_n2o_thermo:.1f} g N₂O/lote\n'
                              f'Redução: {reducao_n2o:.1f}%',
                transform=ax2.transAxes, verticalalignment='top',
                bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))
        
        st.pyplot(fig_n2o_comparacao)
        
        # 2. GRÁFICO DE COMPARAÇÃO CUMULATIVA
        st.subheader("📊 Comparação Cumulativa das Emissões")
        
        # Calcular emissões cumulativas
        cum_ch4_vermi = np.cumsum(ch4_vermi_dia)
        cum_ch4_thermo = np.cumsum(ch4_thermo_dia)
        cum_n2o_vermi = np.cumsum(n2o_vermi_dia)
        cum_n2o_thermo = np.cumsum(n2o_thermo_dia)
        
        fig_cum, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
        
        # Gráfico de CH4 cumulativo
        ax1.plot(dias_array, cum_ch4_vermi, 'b-', linewidth=2, label='Vermicompostagem')
        ax1.plot(dias_array, cum_ch4_thermo, 'r-', linewidth=2, label='Compostagem Termofílica')
        ax1.fill_between(dias_array, cum_ch4_vermi, cum_ch4_thermo, where=(cum_ch4_thermo > cum_ch4_vermi),
                        alpha=0.3, color='green', label='Diferença')
        ax1.set_xlabel('Dia de Compostagem')
        ax1.set_ylabel('CH₄ Acumulado (g)')
        ax1.set_title('Emissões Acumuladas de CH₄')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        # Gráfico de N2O cumulativo
        ax2.plot(dias_array, cum_n2o_vermi, 'b-', linewidth=2, label='Vermicompostagem')
        ax2.plot(dias_array, cum_n2o_thermo, 'r-', linewidth=2, label='Compostagem Termofílica')
        ax2.fill_between(dias_array, cum_n2o_vermi, cum_n2o_thermo, where=(cum_n2o_thermo > cum_n2o_vermi),
                        alpha=0.3, color='green', label='Diferença')
        ax2.set_xlabel('Dia de Compostagem')
        ax2.set_ylabel('N₂O Acumulado (g)')
        ax2.set_title('Emissões Acumuladas de N₂O')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        ...
        # 3. GRÁFICO DE CO₂-EQUIVALENTE
        st.subheader("🌍 Emissões em CO₂-Equivalente")
        
        # Calcular CO₂-equivalente usando GWP do script
        co2eq_ch4_vermi = ch4_vermi_dia * GWP_CH4_20 / 1000  # kg CO₂eq/dia
        co2eq_ch4_thermo = ch4_thermo_dia * GWP_CH4_20 / 1000
        co2eq_n2o_vermi = n2o_vermi_dia * GWP_N2O_20 / 1000
        co2eq_n2o_thermo = n2o_thermo_dia * GWP_N2O_20 / 1000
        
        co2eq_total_vermi = co2eq_ch4_vermi + co2eq_n2o_vermi
        co2eq_total_thermo = co2eq_ch4_thermo + co2eq_n2o_thermo
        
        fig_co2eq, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
        
        # Gráfico de barras comparativo
        labels = ['Vermicompostagem', 'Compostagem Termofílica']
        ch4_values = [co2eq_ch4_vermi.sum(), co2eq_ch4_thermo.sum()]
        n2o_values = [co2eq_n2o_vermi.sum(), co2eq_n2o_thermo.sum()]
        
        x = np.arange(len(labels))
        width = 0.35
        
        ax1.bar(x - width/2, ch4_values, width, label='CH₄', color='orange', alpha=0.8)
        ax1.bar(x + width/2, n2o_values, width, label='N₂O', color='purple', alpha=0.8)
        ax1.set_xlabel('Tecnologia')
        ax1.set_ylabel('Emissões (kg CO₂eq/lote)')
        ax1.set_title('Emissões Totais em CO₂-Equivalente por Gás')
        ax1.set_xticks(x)
        ...
        for i, (ch4, n2o) in enumerate(zip(ch4_values, n2o_values)):
            ax1.text(i - width/2, ch4 + max(max(ch4_values), max(n2o_values))*0.01, 
                    f'{ch4:.1f}', ha='center', fontweight='bold')
            ax1.text(i + width/2, n2o + max(max(ch4_values), max(n2o_values))*0.01, 
                    f'{n2o:.1f}', ha='center', fontweight='bold')
        
        # Gráfico de redução percentual
        reducao_total = ((co2eq_total_thermo.sum() - co2eq_total_vermi.sum()) / co2eq_total_thermo.sum()) * 100
        
        categorias = ['CH₄', 'N₂O', 'Total']
        reducoes = [
            ((co2eq_ch4_thermo.sum() - co2eq_ch4_vermi.sum()) / co2eq_ch4_thermo.sum()) * 100,
            ((co2eq_n2o_thermo.sum() - co2eq_n2o_vermi.sum()) / co2eq_n2o_thermo.sum()) * 100,
            reducao_total
        ]
        
        colors = ['orange', 'purple', 'green']
        bars = ax2.bar(categorias, reducoes, color=colors, alpha=0.8)
        ax2.set_ylabel('Redução (%)')
        ax2.set_title('Redução Percentual na Vermicompostagem')
        ...
        for bar, reducao in zip(bars, reducoes):
            ...
            ax2.text(bar.get_x() + bar.get_width()/2., height + 1,
                    f'{reducao:.1f}%', ha='center', fontweight='bold')
        
        plt.tight_layout()
        st.pyplot(fig_co2eq)
        
        # 4. GRÁFICO DE CONTRIBUIÇÃO RELATIVA
        st.subheader("📋 Contribuição Relativa das Emissões")
        
        # Calcular contribuição percentual
        contrib_ch4_vermi = (co2eq_ch4_vermi.sum() / co2eq_total_vermi.sum()) * 100
        contrib_n2o_vermi = (co2eq_n2o_vermi.sum() / co2eq_total_vermi.sum()) * 100
        contrib_ch4_thermo = (co2eq_ch4_thermo.sum() / co2eq_total_thermo.sum()) * 100
        contrib_n2o_thermo = (co2eq_n2o_thermo.sum() / co2eq_total_thermo.sum()) * 100
        
        fig_contrib, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
        
        # Gráfico de pizza para vermicompostagem
        sizes_vermi = [contrib_ch4_vermi, contrib_n2o_vermi]
        labels_vermi = [f'CH₄\n{contrib_ch4_vermi:.1f}%', f'N₂O\n{contrib_n2o_vermi:.1f}%']
        colors_vermi = ['#FFA500', '#800080']
        
        ax1.pie(sizes_vermi, labels=labels_vermi, colors=colors_vermi, autopct='%1.1f%%', startangle=90)
        ax1.set_title('Contribuição das Emissões - Vermicompostagem')
        
        # Gráfico de pizza para compostagem termofílica
        sizes_thermo = [contrib_ch4_thermo, contrib_n2o_thermo]
        labels_thermo = [f'CH₄\n{contrib_ch4_thermo:.1f}%', f'N₂O\n{contrib_n2o_thermo:.1f}%']
        colors_thermo = ['#FF8C00', '#9932CC']
        
        ax2.pie(sizes_thermo, labels=labels_thermo, colors=colors_thermo, autopct='%1.1f%%', startangle=90)
        ax2.set_title('Contribuição das Emissões - Compostagem Termofílica')
        
        plt.tight_layout()
        st.pyplot(fig_contrib)
        
        # Resumo dos resultados do script
        with st.expander("📋 Resumo dos Resultados do Simulador"):
            st.markdown(f"""
            **📊 Resultados das Simulações do Script:**
            
            **Parâmetros Utilizados:**
            - Resíduos: {residuos_kg_dia} kg/dia
            - Umidade: {umidade_valor}%
            - Temperatura: {T}°C
            - DOC: {DOC}
            
            **Emissões por Lote (50 dias):**
            - **CH₄ Vermicompostagem:** {total_ch4_vermi:.1f} g CH₄/lote
            - **CH₄ Compostagem Termofílica:** {total_ch4_thermo:.1f} g CH₄/lote
            - **Redução CH₄:** {reducao_ch4:.1f}%
            
            - **N₂O Vermicompostagem:** {total_n2o_vermi:.1f} g N₂O/lote
            - **N₂O Compostagem Termofílica:** {total_n2o_thermo:.1f} g N₂O/lote
            - **Redução N₂O:** {reducao_n2o:.1f}%
            
            **Emissões em CO₂-Equivalente (por lote):**
            - **Vermicompostagem:** {co2eq_total_vermi.sum():.1f} kg CO₂eq
            - **Compostagem Termofílica:** {co2eq_total_thermo.sum():.1f} kg CO₂eq
            - **Redução Total:** {reducao_total:.1f}%
            
            **Contribuição Relativa:**
            - **Vermicompostagem:** CH₄ {contrib_ch4_vermi:.1f}%, N₂O {contrib_n2o_vermi:.1f}%
            - **Compostagem Termofílica:** CH₄ {contrib_ch4_thermo:.1f}%, N₂O {contrib_n2o_thermo:.1f}%
            
            **🇧🇷 Implicações para o Brasil (Tabela 18):**
            - Redução de {reducao_total:.1f}% nas emissões de GEE
            - Contribuição para metas brasileiras de descarbonização
            - Benefícios econômicos com créditos de carbono
            - Produção adicional de húmus e minhocas
            """)
